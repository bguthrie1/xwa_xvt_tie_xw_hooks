...
bool PatchMemory(const HookPatch& patch)
{
	...
	if (error)
	{
		...
		MessageBoxA(nullptr, message.c_str(), "xw_hook_main (DInput.dll)", MB_OK | MB_ICONERROR);
		return false;
	}
...
{
	MessageBoxA(nullptr, "The called hook was not correctly installed or initialized.", "xw_hook_main (DInput.dll)", MB_OK | MB_ICONERROR);
